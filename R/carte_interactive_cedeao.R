...
carte_interactive_cedeao <- function(base_senegal,
  ...
  # Vérifier s'il y a des valeurs valides
  ...
  leaflet::leaflet() %>%
    ...
    leaflet::addPolygons(
      ...
      opacity = 1,
      fillOpacity = 0.7,
      popup = ~paste0("<b>", admin0Name, "</b><br>Consommation moyenne annuelle par tête : ", consommation_mer_kg, " kg/an")
    ) %>%
    leaflet::addPolygons(
      data = shp_senegal,
